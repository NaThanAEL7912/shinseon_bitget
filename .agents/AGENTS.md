...
## 3. 쫄다구 코더 완공 보고 시 100% 전수 직접 검수 의무 (폐하 어명 규칙)

* **개발 완료 직후 100% 전수 직접 검수**: 쫄다구 코더 에이전트(Subagent)가 개발 완공을 보고하면 절대로 맹목적으로 승인하지 않으며, 소신이 직접 `view_file`, `git status`, `git diff`, `run_command` 등을 통해 쫄다구가 고친 소스코드의 모든 줄, AWS 서버 반영 상태, 로그 정상 작동 여부를 100% 철저하게 2차 정밀 검수(Code Audit)하여 팩트를 확인한 후 폐하께 보고해야 하옵니다.
